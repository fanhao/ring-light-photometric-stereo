...
figure; mesh(fitted_height); title('误差高度(像素)')
...
% figure; mesh(Height_d); title('误差分析(像素)');colorbar;% axis equal;
disp(['平均高度误差：' num2str(mean(mean(abs(Height_d)))) '像素']);

figure; mesh(Height_d/scale); title('误差分析(毫米)');colorbar;
